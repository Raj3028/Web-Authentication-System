...
// db.query(`INSERT INTO user SET ?`, { name: name, email: email, mobile: mobile, password: password }, (err, result, fields) => {
//     if (err) { console.log(err) }
//     // else { return res.send({ status: false, message: 'User registered' }); }
//     return res.send(result)
// })

// db.query(`delete from user`, (err, result, fields) => {
//     if (err) { console.log(err) }
// })
// return res.send("Rajesh")
    ...
    try {
        ...
        db.query(`SELECT * from user WHERE (email = "${email}")`, async (err, results) => {
            ...
            if (!results || !pass) {
                ...
                const token = jwt.sign({ payload }, "Secret-Key-Rajesh-8984-06-3028", { expiresIn: "24h" });
